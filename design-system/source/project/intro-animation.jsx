// intro-animation.jsx — BookiDesk splash intro (3.5s · 1080×1920)
// El libro cae con rebote → la pasta se abre → la lente escanea la página
// (líneas se iluminan, indicadores pop) → la pasta se cierra de golpe y nace
// el icono → el wordmark BookiDesk entra con resorte.
const { Stage, useTime, Easing, interpolate, animate, clamp } = window;

const U = 7;                      // 1 unidad del logo (viewBox 96) = 7px
const BOOK = 96 * U;              // 672
const BOOK_LEFT = (1080 - BOOK) / 2;
const BOOK_TOP = 624;

const C = {
  cover: '#722033', spine: '#4d1322', gold: '#d4a94e', cream: '#f2e7cf',
  pageEdge: '#d9c49a', ink: '#331016', lens: '#23374d',
};

// líneas de texto de la página [y, ancho] (x=32)
const ROWS = [[24, 36], [31, 42], [38, 26], [45, 40], [52, 44], [59, 30], [73, 34]];

const SPARKS = [
  { x: 60, y: 110, d: 0 }, { x: 590, y: 80, d: 0.06 }, { x: 20, y: 500, d: 0.12 },
  { x: 630, y: 545, d: 0.03 }, { x: 330, y: -50, d: 0.09 },
];
...
function RowsOverlay({ beamU }) {
  return (
    <svg viewBox="0 0 96 96" width={BOOK} height={BOOK} style={svgStyle}>
      {ROWS.map(([y, w], i) => {
        const lit = beamU > y + 2;
        return (
          <rect key={i} x="32" y={y} width={w} height="3" rx="1.5"
            fill={lit ? C.gold : '#ddc9a1'} opacity={lit ? 0.95 : 0.8} />
        );
      })}
    </svg>
  );
...
function Beam({ beamU }) {
  ...
  return (
    <div style={{
      position: 'absolute', left: 22 * U, top: 14 * U, width: 62 * U, height: 72 * U,
      borderRadius: 8 * U, overflow: 'hidden', pointerEvents: 'none',
    }}>
      <div style={{
        position: 'absolute', left: 0, right: 0, top: top - 70, height: 140,
        background: 'linear-gradient(180deg, rgba(212,169,78,0) 0%, rgba(212,169,78,0.32) 50%, rgba(212,169,78,0) 100%)',
      }} />
      <div style={{
        position: 'absolute', left: 0, right: 0, top: top - 2, height: 4,
        background: 'rgba(212,169,78,0.9)', boxShadow: '0 0 18px 4px rgba(212,169,78,0.55)',
      }} />
    </div>
  );
}

function IntroScene({ showTagline, light }) {
  const t = useTime();
  ...
  const p3 = interpolate([2.3, 2.38, 2.54], [0, 1, 0])(t); // impacto al cerrar
  ...
  const lean = interpolate([0.75, 0.85, 1.0], [0, -3, 0], Easing.easeInOutQuad)(t); // anticipación
  ...
  const tagO = animate({ from: 0, to: 1, start: 2.95, end: 3.2, ease: Easing.easeOutQuad })(t);
  const tagY = animate({ from: 18, to: 0, start: 2.95, end: 3.2, ease: Easing.easeOutCubic })(t);

  const glowOp = animate({ from: 0.15, to: light ? 0.55 : 0.9, start: 0.9, end: 2.4, ease: Easing.easeInOutQuad })(t);
  const glowS = 1 + 0.04 * Math.sin(t * 2.1);

  const wordColor = light ? '#33141c' : C.cream;
  const deskColor = light ? '#a5813a' : C.gold;

  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      {/* halo dorado */}
      <div style={{
        position: 'absolute', left: 540 - 680, top: 890 - 680, width: 1360, height: 1360,
        borderRadius: '50%', opacity: glowOp, transform: `scale(${glowS})`,
        background: 'radial-gradient(circle, rgba(212,169,78,0.14) 0%, rgba(212,169,78,0) 62%)',
      }} />

      {/* libro */}
      <div style={{
        position: 'absolute', left: BOOK_LEFT, top: BOOK_TOP, width: BOOK, height: BOOK,
        ...
        <div style={{ position: 'absolute', inset: 0, transform: `translateY(${fallY + hopY}px)` }}>
          <div style={{
            ...
            transform: `scale(${scaleX}, ${scaleY}) rotate(${rotZ}deg)`, perspective: 2400,
          }}>
            <Pages />
            ...
            <div style={{
              ...
              transformOrigin: `${12.5 * U}px 50%`, transform: `rotateY(${rot}deg)`,
            }}>
              <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden' }}><CoverOuter /></div>
              <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}><CoverInner /></div>
            </div>

            {/* lente */}
            {t > 1.05 && (
              <div style={{ position: 'absolute', inset: 0, transformOrigin: `${54 * U}px ${38 * U}px`, transform: `scale(${lensS})` }}>
        ...
        {/* chispas al cerrar */}
        {t > 2.3 && t < 2.85 && SPARKS.map((s, i) => {
          const b = 2.32 + s.d;
          const pp = interpolate([b, b + 0.12, b + 0.36], [0, 1, 0], Easing.easeOutQuad)(t);
          return (
            <div key={i} style={{
              position: 'absolute', left: s.x, top: s.y, width: 24, height: 24,
      ...
      {t > 2.58 && (
        <div style={{
          ...
          fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 124,
          letterSpacing: '-0.02em', color: wordColor, opacity: wO, transform: `scale(${wS})`,
        }}>
          Booki<span style={{ color: deskColor }}>Desk</span>
        </div>
      )}
      {showTagline && t > 2.95 && (
        <div style={{
          position: 'absolute', left: 0, right: 0, top: 1352, textAlign: 'center',
          fontFamily: "'Space Grotesk', sans-serif", fontWeight: 500, fontSize: 30,
          letterSpacing: '0.34em', textIndent: '0.34em',
          color: light ? 'rgba(51,20,28,0.55)' : 'rgba(242,231,207,0.55)',
...
function TimeLabel() {
  ...
  React.useEffect(() => {
    const el = document.querySelector('[data-booki-root]');
    if (el) el.setAttribute('data-screen-label', 'intro t=' + sec + 's');
  }, [sec]);
  return null;
}
...
function BookiIntro({ background, showTagline, loop }) {
  const bg = background || '#2A1016';
  return (
    <Stage width={1080} height={1920} duration={3.5} background={bg} loop={loop !== false} autoplay>
      <IntroScene showTagline={!!showTagline} light={isLight(bg)} />
      <TimeLabel />
    </Stage>
  );
}

window.BookiIntro = BookiIntro;
